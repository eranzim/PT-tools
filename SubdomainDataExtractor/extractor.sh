...
truncate -s 0 html_comments_u.log
...
for subdomain in $(cat $1);do
    ...
    mkdir -v "$dir_name/websites/$subdomain/80" && timeout 20 wget -P "$dir_name/websites/$subdomain/80" -q -r --no-host-directories --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.75 Safari/537.36" http://$subdomain.$2 && to_file $subdomain "python ../FindComments/find_comments.py -s $dir_name/websites/$subdomain/80" html_comments.log && python ../FindComments/find_comments.py -s -e $dir_name/websites/$subdomain/80 >> html_comments_u.log &
    mkdir -v "$dir_name/websites/$subdomain/443" && timeout 20 wget -P "$dir_name/websites/$subdomain/443" -q -r --no-host-directories --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.75 Safari/537.36" https://$subdomain.$2 && to_file $subdomain "python ../FindComments/find_comments.py -s $dir_name/websites/$subdomain/443" html_comments.log && python ../FindComments/find_comments.py -s -e $dir_name/websites/$subdomain/443 >> html_comments_u.log &
...
sort -u html_comments_u.log > html_comments_u.log
